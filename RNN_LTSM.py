...
from all_own_funct import lin_reg_coef
...
"""
File to build the recurrent neural network
"""


output_folder = os.path.join(os.getcwd(), 'Results_RNN_v4', 'dropout')
...
def train(model_name="Ltsm_0", synth_data=False, target='Reintub',
    ...
    f = open(os.path.join(output_folder,
                          'pickled_objects', 'X_TRAIN_Reintub.txt'), 'rb')
    X_TRAIN = pickle.load(f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'Y_TRAIN_{0}.txt'.format(target)), 'rb')
    Y_TRAIN = pickle.load(f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'X_VAL_{0}.txt'.format(target)), 'rb')
    X_VAL = pickle.load(f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'Y_VAL_{0}.txt'.format(target)), 'rb')
    Y_VAL = pickle.load(f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'x_boolmat_val_{0}.txt'.format(target)), 'rb')
    X_BOOLMAT_VAL = pickle.load(f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'y_boolmat_val_{0}.txt'.format(target)), 'rb')
    Y_BOOLMAT_VAL = pickle.load(f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'no_feature_cols_{0}.txt'.format(target)), 'rb')
    ...
    if not os.path.exists(os.path.join(output_folder, 'logs')):
        os.makedirs(os.path.join(output_folder, 'logs'))
    # init callbacks
    tb_callback = TensorBoard(log_dir=os.path.join(output_folder, 'logs', '{0}_.log'.format(model_name)),
                              histogram_freq=0,
                              write_grads=False,
                              write_images=True,
                              write_graph=True)
    ...
    checkpoint_dir = os.path.join(
        output_folder, 'saved_models', "{0}".format(model_name))
    ...
    model.save(os.path.join(output_folder, 'saved_models',
...
def return_loaded_model(model_name="ltsm_0", output_folder=output_folder):
    ...
    loaded_model = load_model(os.path.join(output_folder, 'saved_models',
                                           model_name))
...
def pickle_objects(target='Reintub', time_steps=24, output_folder=output_folder):
    print('step 2')
    if not os.path.exists(os.path.join(output_folder, 'pickled_objects')):
        os.makedirs(os.path.join(output_folder, 'pickled_objects'))
    ...
    f = open(os.path.join(output_folder,
                          'pickled_objects', 'X_TRAIN_{0}.txt'.format(target)), 'wb')
    pickle.dump(X_TRAIN, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'X_VAL_{0}.txt'.format(target)), 'wb')
    pickle.dump(X_VAL, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'Y_TRAIN_{0}.txt'.format(target)), 'wb')
    pickle.dump(Y_TRAIN, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'Y_VAL_{0}.txt'.format(target)), 'wb')
    pickle.dump(Y_VAL, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'X_TEST_{0}.txt'.format(target)), 'wb')
    pickle.dump(X_TEST, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'Y_TEST_{0}.txt'.format(target)), 'wb')
    pickle.dump(Y_TEST, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'x_boolmat_test_{0}.txt'.format(target)), 'wb')
    pickle.dump(x_boolmat_test, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'y_boolmat_test_{0}.txt'.format(target)), 'wb')
    pickle.dump(y_boolmat_test, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'x_boolmat_val_{0}.txt'.format(target)), 'wb')
    pickle.dump(x_boolmat_val, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'y_boolmat_val_{0}.txt'.format(target)), 'wb')
    pickle.dump(y_boolmat_val, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'no_feature_cols_{0}.txt'.format(target)), 'wb')
    pickle.dump(no_feature_cols, f)
    f.close()

    f = open(os.path.join(output_folder,
                          'pickled_objects', 'features_{0}.txt'.format(target)), 'wb')
