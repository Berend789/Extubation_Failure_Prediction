...
from all_own_funct import y_modelling_12
...
def return_data(synth_data=False, balancer=False, target='Reintub',
    ...
    if synth_data:
        ...
        df = pickle.load(f)
        f.close()
        df = df.reset_index(level='pat_hosp_id', drop=False)
        ...
        df_train = df[df['pat_hosp_id'].isin(train_pat)]
        df_val = df[df['pat_hosp_id'].isin(val_pat)]
        df_test = df[df['pat_hosp_id'].isin(test_pat)]
        print(df_train.info())
        X_TRAIN = df_train[COLUMNS].values
        X_TRAIN = X_TRAIN.reshape(
            int(X_TRAIN.shape[0]/time_steps), time_steps, X_TRAIN.shape[1])

        X_VAL = df_val[COLUMNS].values
        X_VAL = X_VAL.reshape(
            int(X_VAL.shape[0]/time_steps), time_steps, X_VAL.shape[1])

        X_TEST = df_test[COLUMNS].values
        X_TEST = X_TEST.reshape(
            int(X_TEST.shape[0]/time_steps), time_steps, X_TEST.shape[1])

        Y_TRAIN = (df_train[[target]]).values
        Y_TRAIN = Y_TRAIN = Y_TRAIN.reshape(
            Y_TRAIN.shape[0]/time_steps, time_steps, 1)

        Y_VAL = (df_val[[target]]).values
        Y_VAL = Y_VAL.reshape(Y_VAL.shape[0]/time_steps, time_steps, 1)

        Y_TEST = (df_test[[target]]).values
        Y_TEST = Y_TEST.reshape(Y_TEST/time_steps, time_steps, 1)
        # create 3D matrix of different shapes
        MATRIX = df[COLUMNS+[target]].values
        print(int(MATRIX.shape[0]/time_steps))
        ...
        print(y_bool_matrix.shape)
        print(Y_VAL.shape)
        print(100*'-')
        x_val_boolmat = x_bool_matrix[len(
            df_val['Adnum'].unique())::]
        y_val_boolmat = y_bool_matrix[len(
            df_val['Adnum'].unique())::]
        print(x_val_boolmat.shape)
        print(y_val_boolmat.shape)
        y_val_boolmat = y_val_boolmat.reshape(
            y_val_boolmat.shape[0], y_val_boolmat.shape[1], 1)
        print(len(df_test['Adnum'].unique()))
        x_test_boolmat = x_bool_matrix[(
            X_MATRIX.shape[0]-len(df_test['Adnum'].unique()))::]
        y_test_boolmat = y_bool_matrix[(
            Y_MATRIX.shape[0]-len(df_test['Adnum'].unique()))::]
        y_test_boolmat = y_test_boolmat.reshape(
            y_test_boolmat.shape[0], 1, 1)
        print(X_TEST.shape)
        print(Y_TEST.shape)
        print(100*'-')
        print(x_test_boolmat.shape)
...
def build_model(no_feature_cols=None, time_steps=12, output_summary=False):
    ...
    RMS = rmsprop.RMSprop(learning_rate=0.001)
...
def train(model_name="Ltsm_0", synth_data=False, target='Reintub',
          balancer=False, predict=False, return_model=False,
