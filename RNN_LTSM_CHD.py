...
PATH = output_folder = os.path.join(os.getcwd(), 'Results_RNN_CHD')
...
def return_data(synth_data=False, balancer=True, target='Reintub',
    ...
    if synth_data:
        ...
        df = pd.read_hdf(os.path.join('./Results_CHD/',
...
def build_model(no_feature_cols=None, time_steps=12, output_summary=False):
    ...
    x = Masking(mask_value=-100, input_shape=(12, 12))(input_layer)
    x = LSTM(256, return_sequences=True)(x)
    preds = TimeDistributed(Dense(1, activation="sigmoid"))(x)
    model = Model(inputs=input_layer, outputs=preds)

    # , rho=0.9, epsilon=1e-08)
    RMS = rmsprop.RMSProp(learning_rate=0.01, rho=0.9, epsilon=1e-08)
...
def train(model_name="Ltsm_0", synth_data=False, target='Reintub',
    ...
    f = open(f'./pickled_object_CHD/X_TRAIN_{target}.txt', 'rb')
    X_TRAIN = pickle.load(f)
    f.close()

    f = open('./pickled_object_CHD/Y_TRAIN_{0}.txt'.format(target), 'rb')
    Y_TRAIN = pickle.load(f)
    f.close()

    f = open('./pickled_object_CHD/X_VAL_{0}.txt'.format(target), 'rb')
    X_VAL = pickle.load(f)
    f.close()

    f = open('./pickled_object_CHD/Y_VAL_{0}.txt'.format(target), 'rb')
    Y_VAL = pickle.load(f)
    f.close()

    f = open('./pickled_object_CHD/x_boolmat_val_{0}.txt'.format(target), 'rb')
    X_BOOLMAT_VAL = pickle.load(f)
    f.close()

    f = open('./pickled_object_CHD/y_boolmat_val_{0}.txt'.format(target), 'rb')
    ...
    # init callbacks
    tb_callback = TensorBoard(log_dir='./logs_CHD/{0}_.log'.format(model_name),
                              histogram_freq=0,
                              write_grads=False,
                              write_images=True,
                              write_graph=True)
    ...
    checkpoint_dir = "./saved_models_CHD/{0}".format(model_name)
    ...
    model.save(os.path.join(os.getcwd(), 'saved_models_CHD',
...
def return_loaded_model(model_name="ltsm_0"):
    ...
    loaded_model = load_model(os.path.join(os.getcwd(), 'saved_models_CHD',
                                           model_name))
...
def pickle_objects(target='Reintub', time_steps=12):
    ...
    output_folder = os.path.join(os.getcwd(), 'pickled_object_CHD')
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    if not os.path.exists('./pickled_object_CHD/X_TRAIN_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/X_TRAIN_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/X_TRAIN_{0}.txt'.format(target), 'wb')
    ...
    if not os.path.exists('./pickled_object_CHD/X_TRAIN_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/X_TRAIN_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/X_VAL_{0}.txt'.format(target), 'wb')
    pickle.dump(X_VAL, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/Y_TRAIN_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/Y_TRAIN_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/Y_TRAIN_{0}.txt'.format(target), 'wb')
    pickle.dump(Y_TRAIN, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/Y_VAL_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/Y_VAL_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/Y_VAL_{0}.txt'.format(target), 'wb')
    pickle.dump(Y_VAL, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/X_TEST_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/X_TEST_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/X_TEST_{0}.txt'.format(target), 'wb')
    pickle.dump(X_TEST, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/Y_TEST_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/Y_TEST_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/Y_TEST_{0}.txt'.format(target), 'wb')
    pickle.dump(Y_TEST, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/x_boolmat_test_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/x_boolmat_test_{0}.txt'.format(target), 'w+'):
            pass
    f = open(
    ...
    if not os.path.exists('./pickled_object_CHD/y_boolmat_test_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/y_boolmat_test_{0}.txt'.format(target), 'w+'):
            pass
    f = open(
    ...
    if not os.path.exists('./pickled_object_CHD/x_boolmat_val_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/x_boolmat_val_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/x_boolmat_val_{0}.txt'.format(target), 'wb')
    pickle.dump(x_boolmat_val, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/y_boolmat_val_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/y_boolmat_val_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/y_boolmat_val_{0}.txt'.format(target), 'wb')
    pickle.dump(y_boolmat_val, f)
    f.close()

    if not os.path.exists('./pickled_object_CHD/no_feature_cols_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/no_feature_cols_{0}.txt'.format(target), 'w+'):
            pass
    f = open(
    ...
    if not os.path.exists('./pickled_object_CHD/features_{0}.txt'.format(target)):
        with open('./pickled_object_CHD/features_{0}.txt'.format(target), 'w+'):
            pass
    f = open('./pickled_object_CHD/features_{0}.txt'.format(target), 'wb')
...
if __name__ == "__main__":
    K.clear_session()
    if not os.path.exists('./pickled_object_CHD/y_boolmat_val_{0}.txt'.format('Reintub')):
